...
class BeamVisualizer3D:
    def __init__(self, beam):
        ...
        (
            ...
            self._alpha_vals,
            self._lam_vals,
        ) = self._beam.extract_solution_arrays()
    ...
    def plot_cross_sections_matplotlib(self, positions=[0.0, 0.5, 1]):
        ...
        for i, pos_ratio in enumerate(positions):
            ...
            axes[i].plot(
                y_circle, z_circle, 'b--', linewidth=1, alpha=0.7, label='Initiale'
            )
            ...
            axes[i].text(
                0.5,
                -0.25,
                f"({index[i]}) Section à x = {x_pos:.1f} cm",
    ...
    def plot_graph_evol_dofs(self):
        ...
        fig, axes = plt.subplots(
            2,
            int(len(dofs_vals) / 2),
            figsize=(4 * (len(dofs_vals) / 2), 4 * (len(dofs_vals) / 2)),
        )
        fig.suptitle(
            'Évolution des variables le long de la poutre',
            fontsize=16,
            fontweight='bold',
        )
    ...
    def debug_solution(self):
        ...
        for name, vals in variables:
            print(
                f"{name:20s}: min={np.min(vals):8.4f}, max={np.max(vals):8.4f}, "
                f"mean={np.mean(vals):8.4f}, std={np.std(vals):8.4f}"
            )

        # Vérifier si il y a des variations significatives
        has_deformation = (
            np.std(self._u1_vals) > 1e-6
            or np.std(self._u3_vals) > 1e-6
            or np.std(self._gamma_vals) > 1e-6
            or np.std(self._alpha_vals) > 1e-6
        )
