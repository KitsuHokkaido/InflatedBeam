...
class InflatedBeam:
    def __init__(
        ...
        self._A, self._D1, self._D2, self._D3, self._D4 = (
            material.compute_stiffness_parameters(h)
        )
    ...
    def _setup_function_space(self):
        ...
        self._v1, self._v3, self._v_gamma, self._v_alpha, self._v_lam = ufl.split(
            self._v_test
        )

        self._u_sol = fem.Function(self._V)
        self._u1_sol, self._u3_sol, self._gamma_sol, self._alpha_sol, self._lam_sol = (
            ufl.split(self._u_sol)
        )
    ...
    def _setup_variational_form(self):
        ...
        H = (
            2 * alpha_1**2 + alpha_11 * (2 * self._alpha_sol + self._alpha_sol**3)
        ) / (2 - self._alpha_sol**2) ** (3 / 2)

        overline_es_squared = (self._R**2 * alpha_1**2 * ufl.pi) / (
            2 - self._alpha_sol**2
        )

        overline_k11s_squared = (self._R**2 * ufl.pi / 4) * (
            3 * (2 - self._alpha_sol**2) * (alpha_11) ** 2
            - 2 * ufl.sqrt(2 - self._alpha_sol**2) * alpha_11 * H
            + 3 * self._alpha_sol**2 * H**2
        )

        overline_k21s_squared = (self._R**2 * alpha_1**2 * ufl.pi) / (
            2 - self._alpha_sol**2
        )
        ...
        overline_k11s = (
            self._R
            * ufl.pi
            * (self._alpha_sol * H - alpha_11 * ufl.sqrt(2 - self._alpha_sol**2))
        )
        ...
        U_p = (self._R / 2) * (
            ...
            + (
                ...
                + self._D1 * self._alpha_sol**2 * ufl.pi
            )
            * k_p**2
        )
        ...
        U_ps = (self._R * self._A * ufl.pi * e_p * self._R**2 * (alpha_1**2)) / (
            2 - self._alpha_sol**2
        )
        ...
        self._total_potential = (
            self._total_internal_energy
            - self._total_external_work
            + self._lagrange_term
        )
    ...
    def _setup_lagrange_multiplier(self):
        ...
        C = (ufl.cos(self._gamma_sol) * u3_1) / j_p - (
            ufl.sin(self._gamma_sol) * (1 + u1_1)
        ) / j_p
    ...
    def _setup_external_work(self):
        ...
        pressure_work_lateral = (
            p
            * ufl.pi
            * self._R**2
            * (
                self._alpha_sol * ufl.cos(self._gamma_sol)
                + ufl.sqrt(2 - self._alpha_sol**2) * (1 - 2 * self._alpha_sol)
            )
        )
        ...
        pressure_work_border = (
            p * ufl.pi * self._R**2 * self._alpha_sol * ufl.sqrt(2 - self._alpha_sol**2)
        ) * (x1_0 - x1_L)
    ...
    def _apply_load_factor(self, factor, original_c_gamma, original_f1, original_f3):
        ...
        self._point_moment = [
            (pos, moment * factor) for pos, moment in self._pt_moment_ref
        ]

        self._point_forces = [
            (pos, f1 * factor, f3 * factor) for pos, f1, f3 in self._pt_forces_ref
        ]
    ...
    def set_initial_geometry(self, geometry_type, **kwargs):
        ...
        if geometry_type == "cylinder":
            ...
            for dof in alpha_dofs:
                ...
                imperfection = amplitude * np.exp(
                    -(((x_coord - position) / width) ** 2)
                )
                self._u_sol.x.array[dof] = 1.0 + imperfection

    def set_external_loads(
        self,
        p: Optional[Union[float, Callable]] = None,
    ...
    def set_boundary_conditions(self, conditions_type: str = 'left_clamped') -> None:
        ...
        boundary_facets_left = mesh.locate_entities_boundary(
            self._domain, self._tdim - 1, boundary_left
        )
        boundary_facets_right = mesh.locate_entities_boundary(
            self._domain, self._tdim - 1, boundary_right
        )

        dofs_left = [
            fem.locate_dofs_topological(
                self._V.sub(i), self._tdim - 1, boundary_facets_left
            )
            for i in range(4)
        ]
        ...
        bc_left = [
            fem.dirichletbc(
                fem.Constant(self._domain, default_scalar_type(bc_left_values[i])),
                dofs_left[i],
                self._V.sub(i),
            )
            for i in range(4)
        ...
        dofs_right = [
            fem.locate_dofs_topological(
                self._V.sub(i), self._tdim - 1, boundary_facets_right
            )
            for i in range(4)
        ]
        ...
        bc_right = [
            fem.dirichletbc(
                fem.Constant(self._domain, default_scalar_type(bc_right_values[i])),
                dofs_right[i],
                self._V.sub(i),
            )
            for i in range(4)
        ...
        if conditions_type == 'left_clamped':
            ...
            bc_dofs_alpha = fem.dirichletbc(
                fem.Constant(self._domain, default_scalar_type(1.0)),
                dofs_alpha,
                self._V.sub(3),
            )
    ...
    def solve(self, load_steps):  # =[0.1, 0.3, 0.6, 1.0]):
        ...
        for i, factor in enumerate(load_steps):
            print(
                f"Étape de charge {i+1}/{len(load_steps)}: {factor*100:.0f}% de la charge"
            )
